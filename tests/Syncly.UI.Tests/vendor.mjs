...
const assets = path.resolve(root, 'src/Syncly.UI/wwwroot');
const symbols = path.join(here, 'node_modules/@material-symbols/svg-400/outlined');
const check = process.argv.includes('--check');

function namesFrom(source) {
  // Pull every "snake_case" string literal out of the icon catalogs.
  return [...source.matchAll(/"([a-z][a-z0-9_]*)"/g)].map((m) => m[1]);
}

const pageIcons = namesFrom(await readFile(path.join(root, 'src/Syncly.Model/PageIcons.cs'), 'utf8'));
const uiIcons = namesFrom(await readFile(path.join(root, 'src/Syncly.Model/MaterialIcons.cs'), 'utf8'));
const material = [...new Set([...pageIcons, ...uiIcons])].sort();
...
for (const name of material) {
  const file = path.join(symbols, `${name}.svg`);
  try {
    await access(file);
    output.set(`icons/material/${name}.svg`, await readFile(file, 'utf8'));
  } catch {
    missing.push(name);
  }
}
if (missing.length) {
  throw new Error(`Material Symbols missing (${missing.length}): ${missing.join(', ')}`);
}
...
console.log(`${check ? 'Verified' : 'Built'} ${output.size} offline assets (${material.length} material icons).`);
